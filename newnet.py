...
def get_args():
    ...
    parser.add_argument('-num_Dec_layer',  type=int, default=4)
...
def build_decoder(args):
    ...
    combined_x = Bidirectional(GRU(units=args.num_Dec_unit, activation='tanh', dropout=dropout_rate,
                                            return_sequences=True, trainable=ont_pretrain_trainable),
                                       name = 'Dec_'+args.rnn_setup+'_'+str(1))(combined_x)
    combined_x = BatchNormalization(name = 'Dec_bn'+'_'+str(1), trainable=ont_pretrain_trainable)(combined_x)

    combined_x = Bidirectional(GRU(units=args.num_Dec_unit, activation='tanh', dropout=dropout_rate,
                                            return_sequences=True, trainable=ont_pretrain_trainable),
                                       name = 'Dec_'+args.rnn_setup+'_'+str(2))(combined_x)
    combined_x = BatchNormalization(name = 'Dec_bn'+'_'+str(2), trainable=ont_pretrain_trainable)(combined_x)
    
    combined_x = Bidirectional(GRU(units=args.num_Dec_unit, activation='tanh', dropout=dropout_rate,
                                            return_sequences=True, trainable=ont_pretrain_trainable),
                                       name = 'Dec_'+args.rnn_setup+'_'+str(3))(combined_x)
    combined_x = BatchNormalization(name = 'Dec_bn'+'_'+str(3), trainable=ont_pretrain_trainable)(combined_x)
                                       
    combined_x = Bidirectional(GRU(units=args.num_Dec_unit, activation='tanh', dropout=dropout_rate,
                                            return_sequences=True, trainable=ont_pretrain_trainable),
                                       name = 'Dec_'+args.rnn_setup+'_'+str(4))(combined_x)
    combined_x = BatchNormalization(name = 'Dec_bn'+'_'+str(4), trainable=ont_pretrain_trainable)(combined_x)
...
def train(args):
    ...
    X_conv_train2 = np.genfromtxt("./X_conv_train.csv")
    X_conv_test2  = np.genfromtxt("./X_conv_val.csv")
    X_train2 = np.genfromtxt("./X_train.csv") 
    X_test2 = np.genfromtxt("./X_val.csv")
    X_conv_train2=2.0*X_conv_train2-1.0
    X_conv_test2=2.0*X_conv_test2-1.0

    X_conv_train=X_conv_train2.reshape((args.num_block,args.block_len, 2))
    X_conv_test=X_conv_test2.reshape((args.num_block/args.test_ratio,args.block_len, 2))
    X_train=X_train2.reshape((args.num_block, args.block_len, 1))
    X_test=X_test2.reshape((args.num_block/args.test_ratio, args.block_len, 1))
    ...
    model.save_weights('./tmp1/conv_dec'+args.id+'.h5')

def test(args, dec_weight):
    X_conv_test2  = np.genfromtxt("./X_conv_test.csv") 
    X_test2 = np.genfromtxt("./X_test.csv")
    ...
    for idx, snr_db in enumerate(SNRS_dB):
        ...
        x = Bidirectional(GRU(units=args.num_Dec_unit, activation='tanh',return_sequences=True,
                                    trainable=dec_trainable),name = 'Dec_'+args.rnn_setup+'_'+str(1))(x)
        x = BatchNormalization(name = 'Dec_bn'+'_'+str(1), trainable=dec_trainable)(x)

        x = Bidirectional(GRU(units=args.num_Dec_unit, activation='tanh',return_sequences=True,
                                    trainable=dec_trainable),name = 'Dec_'+args.rnn_setup+'_'+str(2))(x)
        x = BatchNormalization(name = 'Dec_bn'+'_'+str(2), trainable=dec_trainable)(x)
    
        x = Bidirectional(GRU(units=args.num_Dec_unit, activation='tanh',return_sequences=True,
                                    trainable=dec_trainable),name = 'Dec_'+args.rnn_setup+'_'+str(3))(x)
        x = BatchNormalization(name = 'Dec_bn'+'_'+str(3), trainable=dec_trainable)(x)
        
        y=x                               
        y = Bidirectional(GRU(units=args.num_Dec_unit, activation='tanh',return_sequences=True,
                                    trainable=dec_trainable),name = 'Dec_'+args.rnn_setup+'_'+str(4))(y)
        x = BatchNormalization(name = 'Dec_bn'+'_'+str(4), trainable=dec_trainable)(y)
...
if __name__ == '__main__':
    ...
    train(args)
    test(args, dec_weight='./tmp1/conv_dec'+args.id+'.h5')
